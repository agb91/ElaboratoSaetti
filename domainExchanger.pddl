(define (domain CAMPI)
   ...
   (:predicates 
		...
		(coupleAra ?i - ARATRO ?t - TRA)
		(coupleSemina ?i - SEMINATORE ?t - TRA)
		(seminato ?c - CAMPO)
		(arato ?c - CAMPO)
		(innaffiato ?c - CAMPO)
   ...
   (:action goOnTractor
      ...
      :precondition (and (at ?t ?campo) (at ?c ?campo))   ;;IPOTESI NON METTERA MAI 2 CONTADINI SU TRATTORE PERCHÈ È USELESS
   ...
   (:action linkAra  
	  :parameters (?i - ARATRO ?campo - CAMPO ?c - contadino ?t - TRA-ARA ?i2 - SEMINATORE)
	  :precondition (and (not (coupleSemina ?i2 ?t)) (at ?i ?campo) (at ?c ?campo) (at ?t ?campo) )
	  :effect (and (not (at ?i ?campo)) (coupleAra ?i ?t))
   )
   
   ;;serve che il contadino scenda dal trattore per agganciare l'attrezzo, il contadino non può essere sul trattore
   ;; il seminatore sparisce dal campo quando si lega al trattore (riappare quando si sgancia)
   (:action linkSemina  
	  :parameters (?i - SEMINATORE ?campo - CAMPO ?c - contadino ?t - TRA-SEMINA ?i2 - ARATRO)
	  :precondition (and (not (coupleAra ?i2 ?t)) (at ?i ?campo) (at ?c ?campo) (at ?t ?campo))
	  :effect (and (not (at ?i ?campo)) (coupleSemina ?i ?t))
   )
   
   (:action unLinkAra 
	  :parameters (?i - ARATRO ?campo - CAMPO ?c - contadino ?t - TRA-ARA)
	  :precondition (and (coupleAra ?i ?t) (at ?t ?campo) (at ?c ?campo)) 
	  :effect (and (at ?i ?campo) (not (coupleAra ?i ?t)))
   )
   
   (:action unLinkSemina 
	  :parameters (?i - SEMINATORE ?campo - CAMPO ?c - contadino ?t - TRA-SEMINA)
	  :precondition (and (coupleSemina ?i ?t) (at ?t ?campo) (at ?c ?campo)) 
	  :effect (and (at ?i ?campo) (not (coupleSemina ?i ?t)))
   )
